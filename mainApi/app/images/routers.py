...
from mainApi.app.images.utils.cellpose.tissue import process_TissueNT_Segmentation,getBinaryImage,getMergedImageByWholeChannels,getResultImageFromColorOptions,TISSUE_COLORS,getDotPlotImage,processMeasureForTissueNT,process_TissueNT_Test_Segmentation,getMergedImage,processForAllChannels
...
import cv2
import math
from mainApi.app.images.utils.cellpose.label_free import predict_mask
from mainApi.app.images.utils.cellpose.process_binary import convert2Binary, convertBinary2SelectedColorImage
from mainApi.app.images.utils.cellpose.cellpose_segment import getExtractAllClasses, cropSegmentsFromOriginalImage, random_color
...
async def processHeatMap(
    ...
    addROIInHeatMapImage(work_dir + "1.png",work_dir + "roi_1.png", [25,45,55,75])
    addROIInHeatMapImage(work_dir + "2.png",work_dir + "roi_2.png", [27,47,58,78])
    addROIInHeatMapImage(work_dir + "3.png",work_dir + "roi_3.png", [28,48,55,75])
    addROIInHeatMapImage(work_dir + "4.png",work_dir + "roi_4.png", [30,50,55,75])
    addROIInHeatMapImage(work_dir + "5.png",work_dir + "roi_5.png", [25,45,55,75])
...
@router.post(
    "/ml_label_free_process",
    response_description="Label Free Method process",
    status_code=status.HTTP_200_OK,
)
async def processLabelFreeProcess(
    request: Request,
    user: UserModelDB = Depends(get_current_user), 
):
    ...
    url = data.get("original_image_url")
    sensitivity = data.get("sensitivity")

    filename = url.split('/')[-1]

    url_first_part = url.split("download/?path=")[0]
    temp_path = url.split("download/?path=")[-1]

    imagePath = '/app/mainApi/app/static/' + temp_path

    real_image_path = imagePath.replace(".ome.tiff", ".jpg")
    print(real_image_path)

    orgImage = cv2.imread(real_image_path)
    orgImage = cv2.resize(orgImage,(512,512))

    #Directoy Path of original image
    dirPath = imagePath.split(filename)[0]

    #This will be directory to save the result of the label free result
    labelDirPath = os.path.join(dirPath, "labelFree")

    if os.path.exists(labelDirPath) == False:
        os.mkdir(labelDirPath)

    #The file path of the result image by processing the cellpose model

    segmentResultPath = os.path.join(labelDirPath, "segment.jpg")
    binarySegmentImage = os.path.join(labelDirPath, "segmentBinary.jpg")
    print("The result path of segmented image is ", segmentResultPath)
    binImage =  None

    if os.path.exists(segmentResultPath) == False or os.path.exists(binarySegmentImage):
        masks, flows = predict_mask(orgImage)
        cv2.imwrite(segmentResultPath, masks )
        resultImage = cv2.imread(segmentResultPath)

        binImage = convert2Binary(resultImage)
        cv2.imwrite(binarySegmentImage, binImage)

    else :
        binImage = cv2.imread(binarySegmentImage)

    res_dir_path = os.path.join(labelDirPath, "result")

    tot_res = getExtractAllClasses(binImage)

    height = binImage.shape[0]
    width = binImage.shape[1]

    resImg = np.zeros((height,width,3))

    cropSegmentsFromOriginalImage(orgImage, tot_res, res_dir_path)



    height = binImage.shape[0]
    width = binImage.shape[1]
    color_image_path = os.path.join(labelDirPath, "coloredResult.jpg")
    ...
    cv2.imwrite(color_image_path, result_image)

    coloredOmeTiffPath = color_image_path.replace(".jpg",".ome.tiff")
    print(coloredOmeTiffPath)

    temp_result_path = os.path.join(labelDirPath, "tempResult.jpg")
    tempResultOmeTiffPath = temp_result_path.replace(".jpg", ".ome.tiff")

    bf_cmd = f"sh /app/mainApi/bftools/bfconvert -noflat -separate -tilex 1024 -tiley 1024 -pyramid-scale 2 -pyramid-resolutions 4 -separate -overwrite '{temp_result_path}' '{tempResultOmeTiffPath}'"
    await shell(bf_cmd)

    # convert tiff image to png for thumbnail
    bf_cmd = f"sh /app/mainApi/bftools/bfconvert -noflat -separate -tilex 1024 -tiley 1024 -pyramid-scale 2 -pyramid-resolutions 4 -separate -overwrite '{color_image_path}' '{coloredOmeTiffPath}'"
    await shell(bf_cmd)
    #processTopViewMouseTracking(file_path)

    user_dir_path = coloredOmeTiffPath.split(str(PyObjectId(user.id)))[-1]
    result_image_path = url_first_part + "download/?path=" + str(PyObjectId(user.id)) + user_dir_path

    tempDirPath = tempResultOmeTiffPath.split(str(PyObjectId(user.id)))[-1]
    tempFinalPath = url_first_part + "download/?path=" + str(PyObjectId(user.id)) + tempDirPath

    print(result_image_path)

    return {"result": "success", "file_path" : res_dir_path, "result_image_path" : result_image_path , "temp_image_path": tempFinalPath}



@router.post(
    "/ml_get_label_free_segment_result",
    response_description="Get Label Free Segment Result Images",
    status_code=status.HTTP_200_OK,
)
async def getLabelFreeSegmentResultImages(
    request: Request,
    user: UserModelDB = Depends(get_current_user), 
):
    ...
    for fileName in file_list:
        print(fileName)
        tempPath = os.path.join(dir_path, fileName)
        secondPathPart = tempPath.split('static/')[-1]

        result_image_path = url_first_part + "download/?path=" + secondPathPart
        print(result_image_path)

        res.append(result_image_path)
    
    return {"result": "success", "segment_result_list" : res }
