...
from mainApi.app.images.utils.threeD.util import save_all_frames 
    ...
    for each_file_folder in files:
        ...
        async with aiofiles.open(new_folder_path, "wb") as f:
            ...
            if not each_file_folder.filename.endswith((".ome.tiff", ".ome.tif")):
                ...
                if input.lower().endswith((".tif",".TIFF",".TIF")):
                ...
                print(input)
                if not input.lower().endswith((".avi")):
                    if input.lower().endswith((".png",".PNG",".jpg",'.JPG',".jpeg",".JPEG",".tif",".TIF")):
                        bfconv_cmd = f"sh /app/mainApi/bftools/bfconvert -noflat -separate -tilex 1024 -tiley 1024 -pyramid-scale 2 -pyramid-resolutions 4 -overwrite '{input}' '{output}'"

                    else :
                        bfconv_cmd = f"sh /app/mainApi/bftools/bfconvert -noflat -no-upgrade -tilex 1024 -tiley 1024 -pyramid-scale 2  -pyramid-resolutions 4 -overwrite '{input}' '{output}'"
                    
                    await shell(bfconv_cmd)
                else:
                    print(folder)
                    result_folder = os.path.join(folder, "result_images_dir")
                    save_all_frames(input, result_folder, "cut")

                    num_of_count = 20
                    final_path = os.path.join( folder ,"./temp.tiff")
                    files = [f for f in os.listdir(result_folder) if os.path.isfile(os.path.join(result_folder, f)) and f.endswith(".tiff")]

                    files.sort()
                    img1 = tifffile.imread(os.path.join(result_folder, files[0]))

                    zstack = np.zeros((1,num_of_count ,3, img1.shape[0], img1.shape[1]), 'uint8')
                    ...
                    with tifffile.TiffWriter(final_path, imagej=True) as tif:
                        tif.write(zstack)
                    
                    bfconv_cmd = f"sh /app/mainApi/bftools/bfconvert -noflat -no-upgrade -tilex 1024 -tiley 1024 -pyramid-scale 2  -pyramid-resolutions 4 -overwrite '{final_path}' '{output}'"
                ...
                try:
                    ...
                    z0_channel_normalized = (int)(z0_channel - np.min(z0_channel)) / (np.max(z0_channel) - np.min(z0_channel)) * 255
...
async def add_experiment_with_folders_with_video(
    folderPath: str,
    experiment_name: str,
    files: List[UploadFile],
    paths: List[str],
