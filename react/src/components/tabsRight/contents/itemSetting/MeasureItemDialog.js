...
import { processBasicMeasure } from '@/api/measure';
...
const MeasureItemDialog = (props) => {
  ...
  // const doBasicMeasurement = async (path) => {
  //   await processBasicMeasure(path);
  // };
  ...
  useEffect(() => {
    ...
    if (path) {
      //console.log(path);
      //add the record for new measurement items here
      // Basic measurement items
      //console.log(path);
      //doBasicMeasurement(path);
